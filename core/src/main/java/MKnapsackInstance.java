...
public class MKnapsackInstance {
    ...
    public static void main(String[] args) {
        ...
        int[] W = {800,650,550,550,650};

        MKnapsack mKnapsack = new MKnapsack(prepare(data), W, 50);
        mKnapsack.solve();
    }

    private static int[][] prepare(int[][] data){
        int[][] reversed = new int[data[0].length][data.length+1];

        for(int i = 0; i < reversed.length; i++) {
            for(int j = 1; j < reversed[i].length; j++) reversed[i][j] = data[j-1][i];
            reversed[i][0] = 1;
        }
